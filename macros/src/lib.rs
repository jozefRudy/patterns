...
use syn::{Data, DeriveInput, Error, Expr, Field, Fields, Ident, Lit, LitStr, Meta, Result, Token};
    ...
    Ok(quote! {
        ...
        impl ::patterns::llm_cli::Extractable for #name {
            ...
            fn render_prompt(
                ...
                #prompt_struct { schema, text, prompt_context }
                    .render()
...
/// Container args: `#[systemone(template = "...", healthcheck = "...")]`.
struct Container {
    template: LitStr,
    healthcheck: Option<LitStr>,
}

fn parse_container(attr: &syn::Attribute) -> Result<Container> {
    ...
    Ok(quote! {
        ...
        impl ::patterns::systemone::Questions for #name {
            ...
            fn render_state(text: &str, prompt_context: &str) -> ::anyhow::Result<String> {
                use ::patterns::askama::Template as _;
                #template_struct { text, prompt_context }
                    .render()
                    .map_err(::core::convert::Into::into)
            }
        }
